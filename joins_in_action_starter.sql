...
SELECT distinct lead_studio, count(favorite_movie_id) FROM movies LEFT JOIN users on movie_id = favorite_movie_id group by lead_studio order by count(favorite_movie_id) desc limit 1;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
-- EXPECTED RESULT: 52.21
SELECT avg(rotten_tomatoes) FROM movies LEFT JOIN users on movie_id = favorite_movie_id 
UNION
SELECT avg(rotten_tomatoes) FROM movies RIGHT JOIN users on movie_id = favorite_movie_id ;
...
Select film, count(film) from movies group by film;
Select film, count(film) from movies  group by film having count(film) >1;
